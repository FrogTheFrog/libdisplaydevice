...
namespace {
  ...
  public:
    ...
    getImpl() {
      if (!m_impl) {
        m_impl = std::make_unique<display_device::SettingsManager>(m_dd_api, m_settings_persistence_api, m_audio_context_api);
      }
...
#define TEST_F_S_MOCKED(...) DD_MAKE_TEST(TEST_F, SettingsManagerGeneralMocked, __VA_ARGS__)
  ...
  const std::optional<std::vector<std::uint8_t>> NULL_DATA { std::nullopt };
  const std::optional<std::vector<std::uint8_t>> EMPTY_DATA { std::vector<std::uint8_t> {} };
}  // namespace
...
TEST_F_S_MOCKED(FailedToLoadPersitence) {
  EXPECT_CALL(*m_settings_persistence_api, load())
    .Times(1)
    .WillOnce(Return(NULL_DATA));

  EXPECT_THAT([this]() { getImpl(); }, ThrowsMessage<std::runtime_error>(HasSubstr("Failed to load persistent settings!")));
}
...
TEST_F_S_MOCKED(InvalidPersitenceData) {
  ...
  EXPECT_THAT([this]() { getImpl(); }, ThrowsMessage<std::runtime_error>(HasSubstr("Failed to parse persistent settings! Error:\n"
