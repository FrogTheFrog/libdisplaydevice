...
namespace {
  ...
  class SettingsManagerMocked: public BaseTest {
  public:
...
#define TEST_F_S_MOCKED(...) DD_MAKE_TEST(TEST_F, SettingsManagerMocked, __VA_ARGS__)
}  // namespace
...
TEST_F_S_MOCKED(ApplySettings, NoApiAccess) {
  InSequence sequence;
  EXPECT_CALL(*m_dd_api, isApiAccessAvailable())
    .Times(1)
